...
def walk(self):
    ...
    # Comentado para que funcione la migracion de PloneFormGen a EasyForm
    ...
    for brain in brains:
        try:
            obj = brain.getObject()
            #Añadir que si tiene layout le ponga el view porque pfg_draft_view ya no funciona
            if obj.getLayout() != None:
                obj.setLayout('view')
        except AttributeError:
            ...
            for idx, row in enumerate(data_adapter.getSavedFormInput()):
                ...
                for key, value in zip(cols, row):
                    ...
                    if IFromUnicode.providedBy(field):
                        # Añadido try - except para que funcione
                        try:
